  ...
  return (
    <div className="space-y-5">
      ...
      {showNew&&<div className="fixed inset-0 z-50 grid place-items-center p-4"><div className="absolute inset-0 bg-black/25" onClick={()=>setShowNew(false)}/><div className="relative bg-white border border-line rounded-xl p-6 w-full max-w-lg">
        ...
        <form className="mt-5 grid grid-cols-2 gap-3" onSubmit={e=>{e.preventDefault();const result=createEmployee(form);if(result.ok&&result.loginId&&result.temporaryPassword)setCreated({loginId:result.loginId,temporaryPassword:result.temporaryPassword})}}>
